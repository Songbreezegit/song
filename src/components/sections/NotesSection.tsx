...
export function NotesSection({ onSelectArticle, articles, loading, error, onRetry }: { onSelectArticle: (a: Article) => void; articles: Article[] } & Omit<DataStateProps, 'empty'>) {
 const [expanded, setExpanded] = useState(false);
 const [filter, setFilter] = useState('全部');
 const dataList = loading || error ? [] : articles;
 ...
 return <section id="notes" className="section notes-section"><div className="container">
  ...
  <DataState loading={loading} error={error} empty={!articles.length} onRetry={onRetry} emptyMessage="暂无已发布笔记。" />
  <div className="notes-grid">{featuredArticle && <ScrollReveal><NoteCard article={featuredArticle} onSelect={onSelectArticle} featured /></ScrollReveal>}<div className="note-stack">{stackArticles.map((a,i) => <ScrollReveal key={a.id} delayMs={i*75}><NoteCard article={a} onSelect={onSelectArticle} /></ScrollReveal>)}</div></div>
