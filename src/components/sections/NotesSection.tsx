...
export function NotesSection({ onSelectArticle }: { onSelectArticle: (a: Article) => void }) {
 const [expanded, setExpanded] = useState(false);
 const [filter, setFilter] = useState('全部');
 const categories = ['全部', ...new Set(ARTICLES.map(a => a.category))];
 const archive = ARTICLES.slice(4).filter(a => filter === '全部' || a.category === filter);
 return <section id="notes" className="section notes-section"><div className="container">
  ...
  <div className="notes-grid"><ScrollReveal><NoteCard article={ARTICLES[0]!} onSelect={onSelectArticle} featured /></ScrollReveal><div className="note-stack">{ARTICLES.slice(1,4).map((a,i) => <ScrollReveal key={a.id} delayMs={i*75}><NoteCard article={a} onSelect={onSelectArticle} /></ScrollReveal>)}</div></div>
  <div className="notes-bottom"><div><p className="small-label">A GROWING COLLECTION OF THOUGHTS</p><button className="text-link" aria-expanded={expanded} aria-controls="notes-archive" onClick={() => setExpanded(!expanded)}>{expanded ? '收起笔记' : `浏览全部 ${ARTICLES.length} 篇笔记`}<ArrowRight size={18} /></button></div><Asset name="notes-slogan-small-notes" className="notes-slogan" alt="Small Notes. Big Thoughts." /></div>
